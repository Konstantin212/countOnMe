...
`backend/app/features/catalog/models.py` — `CatalogProduct(Base)` and `CatalogPortion(Base)` with `TimestampMixin`.
...
`backend/app/features/catalog/service.py`:
...
`backend/app/features/catalog/router.py`:
...
`seed.py` at the repo root:

```python
#!/usr/bin/env python
"""Seed the catalog. Run from anywhere: python seed.py [--dry-run]"""
os.environ.setdefault("DATABASE_URL", "postgresql://countonme:countonme@localhost:5433/countonme")
backend_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "backend")
sys.exit(subprocess.call([sys.executable, "-m", "scripts.seed_catalog", *sys.argv[1:]], cwd=backend_dir))
```

The `*sys.argv[1:]` passthrough allows `python seed.py --dry-run` to work.
...
| `backend/app/features/catalog/schemas.py` | Pydantic request/response schemas |
| `backend/app/features/catalog/service.py` | Read-only service: list, get, search |
| `backend/app/features/catalog/router.py` | FastAPI router: `GET /v1/catalog/products` |
...
| `seed.py` | Python wrapper at repo root |
